...
## 17. Deploying Agent Router: what the study caught, and what only deploying caught

The internal study (`docs/LiteLLM vs Agent Router (Envoy AI Gateway).docx`) is
the authority for versions and known issues. It corrected two wrong pins before
they reached the cluster:

| | first attempt | study's matrix | deployed |
| --- | --- | --- | --- |
| Envoy Gateway | v1.5.2 | **1.8.1+** | v1.8.1 |
| Gateway API | v1.3.0 | **1.5** | v1.5.1 (from the Envoy Gateway chart) |

v1.5.2 came from reading the go.mod of the Envoy Gateway image. Wrong source of
truth: the floor is set by the **AI Gateway release's** compatibility matrix.

The study's **#2591** (webhook cert generated by a Helm `lookup` → permanent
OutOfSync) was avoided entirely by `controller.mutatingWebhook.certManager.enable`,
which replaces the generated Secret with a cert-manager Issuer + Certificate. No
`ignoreDifferences` anywhere, which the study's POC sets as an exit criterion.

### Five failures that only appeared on deploy

1. **The AppProject is not GitOps-managed.** Adding `docker.io/envoyproxy` to
   `gitops/bootstrap/project.yaml` changed nothing until `kubectl apply`. All
   five Applications failed with `InvalidSpecError: application repo ... is not
   permitted in project 'mlops'` — an error on the *Application*, pointing away
   from the file that needed editing. Now `make project`.

2. **`helm template` hides `crds/`.** Checking whether Envoy Gateway ships the
   Gateway API CRDs with `helm template | grep` returned nothing, so a separate
   CRD Application was added. ArgoCD *always* applies `crds/`, so the two fought
   over the same objects (standard vs experimental channel) and neither reached
   Synced, blocking the root app at wave 1. **Always use
   `helm template --include-crds`** to see what ArgoCD will install.

3. **The Gateway API ships an admission policy that blocks channel changes.**
   Replacing the standard CRDs with Envoy Gateway's experimental ones was denied
   by `ValidatingAdmissionPolicy safe-upgrades.gateway.networking.k8s.io`. Its
   own message gives the remedy: uninstall the policy.

4. **`LoadBalancer` again.** The Envoy proxy Service defaults to LoadBalancer,
   which is `<pending>` forever on kind, so the Gateway stayed
   `Programmed=False / AddressNotAssigned`. `envoyService.type: ClusterIP`.
   Same trap as §12.

5. **The proxy lives in `envoy-gateway-system`, not the Gateway's namespace**,
   under a hash-suffixed Service name. An Ingress can only target a Service in
   its own namespace, and the "stable alias Service" written to work around that
   could never work either — a Service only selects pods in its own namespace.
   Access is a port-forward; discover the Service by
   `gateway.envoyproxy.io/owning-gateway-name`.

### The one that mattered: extensionManager

Everything reported healthy — Gateway `Programmed=True`, AIGatewayRoute,
AIServiceBackend and Backend all `Accepted`, HTTPRoute generated with correct
per-model rules, ext_proc sidecar injected and logging *"ready"* — and every
request still returned:
...
Sending `x-ai-eg-model: qwen3` by hand routed correctly and the model answered.
That isolated it: the header was never being set, because Envoy Gateway was
never calling the AI Gateway controller's **extension server** (grpc/1063). That
is the study's "Envoy Gateway avec les values du projet" step — without
`extensionManager`, the ext_proc filter is never inserted into the filter chain.

One further trap: requesting the `HTTPListener` post-hook makes every
translation fail with `method PostHTTPListenerModify not implemented`, so no xDS
publishes at all. `Translation` is the hook the controller serves.

**No Application status shows any of this.** The whole stack reads green while
returning errors to every caller.

### Result

Both gateways, same prompt, same model, same KServe backend:

```
LiteLLM          1.59s  Kubernetes is a container orchestration system.
Agent Router     0.96s  Kubernetes is a system for managing containerized applications...
```

Single measurement on a loaded CPU node — not the study's p99 benchmark. It shows
the path works, nothing more.
